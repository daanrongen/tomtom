...
const redactApiKey = (apiKey: string | undefined): string | undefined =>
  apiKey ? `${"*".repeat(Math.max(0, apiKey.length - 4))}${apiKey.slice(-4)}` : undefined;

const get = Command.make("get", {}, () =>
  Effect.gen(function* () {
    const store = yield* ConfigStore;
    const stored = yield* store.load;
    yield* Console.log(JSON.stringify({ ...stored, apiKey: redactApiKey(stored.apiKey) }, null, 2));
  }),
).pipe(Command.withDescription("Print the resolved config file (API key redacted)"));

const set = Command.make(
  "set",
  {
    key: Args.text({ name: "key" }).pipe(Args.withDescription(`One of: ${VALID_KEYS.join(", ")}`)),
...
).pipe(Command.withDescription("Set a config value, e.g. `tomtom config set api-key <key>`"));
...
const test = Command.make("test", globalOptions, (flags: GlobalFlags) =>
  withTomTomClient(
    ...
    Effect.gen(function* () {
      ...
      yield* Console.log("OK: the configured API key can reach the TomTom Search API.");
    }),
  ),
).pipe(Command.withDescription("Validate the configured API key against a real TomTom endpoint"));
