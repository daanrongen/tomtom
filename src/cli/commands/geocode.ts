...
import { render, renderGeocodeResults, renderReverseGeocodeResults } from "@/cli/render.js";
...
const countrySetOption = Options.text("country").pipe(
  ...
  Options.withDescription("Comma-separated ISO country codes to bias/restrict results, e.g. GB,FR"),
);
  ...
  (parsed) =>
    withTomTomClient(
      ...
      Effect.gen(function* () {
        ...
        yield* render(parsed, data, renderGeocodeResults as (d: unknown) => string);
      }),
    ),
...
const returnSpeedLimitOption = Options.boolean("return-speed-limit").pipe(Options.withDefault(false));
  ...
  (parsed) =>
    withTomTomClient(
      ...
      Effect.gen(function* () {
        ...
        yield* render(parsed, data, renderReverseGeocodeResults as (d: unknown) => string);
      }),
    ),
